	...
	<div class="acc_list">
		...
		<div class="container">
			...
			<c:forEach var="acc" items="${accList}" varStatus="count">
				<div id="acc_list_contents_${count.index + 1}"
					class="acc_list_contents">
					<div class="acc_list_contain" data-toggle="modal"
						data-target="#staticBackdrop">
						<div class="acc_image">
							<img src="${acc.imgUrl}" alt="">
						</div>
						<!--  onclick="getRoomNo(${acc.roomNo }) -->
						<div class="acc_list_info">
							<input id="roomNoInput" type="text" value="${acc.roomNo }"></input>
							<h4>${acc.roomName}</h4>
					...
					<div class="acc_list_button">
						<button type="button" class="price_btn">${acc.roomPrice }
							KRW</button>
					</div>
				</div>
			</c:forEach>
				...
				<div class="modal-body" style="background-color: #F5F5F5;">
					<div id="carouselExampleFade" class="carousel slide carousel-fade"
						...
						<div class="carousel-inner">
							<div id="imgWrapper" class="carousel-item active"></div>
						</div>
					...
					<div class="acc_detail_div">
						<h3>디럭스 트윈</h3>
						<div>여유로운 휴식 공간이 돋보이는 롯데호텔 부산 디럭스 룸은 끊임 없는 리뉴얼로 최상의 컨디션을
							유지하고 있습니다. 객실 내에 마련된 대형 데스크는 출장지에서도 빈틈 없는 업무 처리를 돕습니다. 창 밖으로 부산 한
							중심의 화려한 시티뷰를 감상하실 수 있는 롯데호텔 부산 디럭스 룸에서 완벽한 여행을 꾸며 보세요.</div>
					</div>
					<div class="acc_detail_exp">
						<div class="acc_detail_info">
							...
							<div>
								...
								<div class="acc_detail_info_content" onclick="a(1);">33.68~36.90
									㎡</div>
							</div>
						</div>
						<div class="br_div"></div>
						<div class="acc_detail_info">
							...
							<div>
								<div class="acc_detail_info_content">
									...
									<div>32'' LCD TV, 손전등, 슬리퍼, 구둣주걱, 구두클리너, 전화기, 티포트, 금고,
										냉장고, 대형옷장, 미니바, 소화기</div>
								</div>
								<div class="acc_detail_info_content">
		 ...
		 $(document).ready(function() {
			    $('.acc_list_contents').each(function(index, element) {
			        $(element).on('click', function() {
			            let roomId = $(element).attr('id');
			            console.log(roomId);
			            getModal(roomId.substr(-1));
			        });
			    });
			});

			function getModal(roomNo) {
				console.log(roomNo);
			    $.get('http://localhost:8080/wherego/acc/detail/' + roomNo, function(data) {
			        console.log(data);
			        let a = document.querySelector('#imgListView');
			        data.forEach((e, idx)=>{
			        	console.log(e);
			        	//$('#imgWrapper').append($('<img id="imgListView'+ idx + 1 +'" src="" class="d-block w-100" alt="">'));
			        	$('#imgWrapper').append($('<img id="imgListView'+ idx + 1 +'" src="" class="d-block w-100" alt="">'));
			        	$('#imgListView' + idx + 1).attr('src',e);
				...
				/* $.ajax({
					type:"GET",
					url: "http://localhost:8080/wherego/acc/detail/" + roomNo,
					data: {
						
					},
					success: function(data) {
						//  data -> 문자열 타입 (json 형식에 단순 문자열) 
						// data ->--> js 언어 이해하는 데이터 자료 구조로 변경 --> object  
						// JSON.stringfy(); 조사 
						/* $.each(data, function(index, value){
							console.log(value);
							// DOM 접근해서 값을 다시 넣기
							$(".imgListView").attr('src', value);
						}); 
						console.log(data);
						data.forEach(e => {
							//console.log($('.d-block w-100').attr('src',e));
						})
					},
					error: function() {
						console.log("오류 발생");
